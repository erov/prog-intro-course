...
public class ExpressionTest extends BaseTest {
    ...
    private void handmade() {
        ...
        testExpression("(x / x)", "x / x", new Divide(new Variable("x"), new Variable("x")), x -> x / x);
        testExpression("(1 - (2 + 3))", "1 - (2 + 3)", new Subtract(new Const(1), new Add(new Const(2), new Const(3))), x -> -4);
        testExpression("(1 / (x / 3))", "1 / (x / 3)", new Divide(new Const(1), new Divide(new Variable("x"), new Const(3))), x -> 1 / (x / 3));
    ...
    private void generated() {
        ...
        testExpression("(x + (2 - 1))", "x + 2 - 1", new Add(new Variable("x"), new Subtract(c2, c1)), x -> (x + (2 - 1)));
